...
IF ws EQ 1 THEN BEGIN
  ...
  ENDIF ELSE BEGIN
    uv=SQRT(0.9225^2)+SQRT(gm*0.444/2.219)
    c=CFL/uv ;courant coeffitient
    dt=dx*c
    nt=FLOOR(1.0/dt)+1
    W=MAKE_ARRAY(nx,nt,7,/DOUBLE)
...
IF ws EQ 2 THEN BEGIN
  ...
  IF fs EQ 1 THEN BEGIN
    uv=SQRT(15.3^2)+SQRT(gm*305.9/3.896)
    c=CFL/uv ;courant coeffitient
    dt=dx*c
    nt=FLOOR(0.1/dt)+1
    nt1=FLOOR(0.05/dt)+1
    ...
    W[count:nx-1,0,*]=TRANSPOSE(REBIN([1, 1,-15.3, 0, 0, 1./SQRT(4*!PI), 4./SQRT(4*!PI)],7,nx-count))
  ENDIF ELSE BEGIN
    uv=SQRT(0.9225^2)+SQRT(gm*1.4336/3.108)
    c=CFL/uv ;courant coeffitient
    dt=dx*c
    nt=FLOOR(1.6/dt)+1
    nt1=FLOOR(0.8/dt)+1
...
IF (fs EQ 1) AND (ws EQ 1) THEN fig_hw4.SAVE,'hw4_lw_1f.pdf',RESOLUTION=512,/TRANSPARENT
